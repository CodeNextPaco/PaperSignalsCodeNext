...
void PaperSignals::TimerExecution(String JSONData)
{
 ...
 //   JsonObject& root = jsonBuffer.parseObject(JSONData);
  ...
  int seconds = jsonBuffer["parameters"]["duration"]["inSeconds"];
...
double PaperSignals::GetBitcoin()
{
    ...
    //  JsonObject& root = jsonBuffer.parseObject(unPretty);
      DeserializationError error = deserializeJson(jsonBuffer, unPretty);
    ...
    double change = jsonBuffer[String("percent_change_24h")];
...
void PaperSignals::CryptoCurrencyExecution(String JSONData)
{
    ...
    DynamicJsonDocument cryptoBuffer(1024);
   //   JsonObject& cryptoRoot = cryptoBuffer.parseObject(JSONData);
        DeserializationError error = deserializeJson(cryptoBuffer, JSONData);
   
    String CurrencyType = cryptoBuffer["parameters"]["Crypto"];
...
{
  DynamicJsonDocument stretchBreakBuffer(1024);
  //  JsonObject& stretchBreakRoot = stretchBreakBuffer.parseObject(JSONData);
    deserializeJson(stretchBreakBuffer, JSONData);
  
  breakTimeInterval = stretchBreakBuffer["parameters"]["duration"]["amount"]; // In Minutes
...
void PaperSignals::ShortsOrPantsExecution(String JSONData)
{
  ...
  DynamicJsonDocument weatherLocationBuffer(1024);
  //  JsonObject& locationRoot = weatherLocationBuffer.parseObject(JSONData);
     deserializeJson(weatherLocationBuffer,JSONData);

  
  String City = weatherLocationBuffer["parameters"]["location"]["city"];
  String State = weatherLocationBuffer["parameters"]["location"]["admin-area"];
  String Address = weatherLocationBuffer["parameters"]["location"]["street-address"];
  String MaybeAlsoACity = weatherLocationBuffer["parameters"]["location"]["subadmin-area"];

  String Location = MaybeAlsoACity + " " + City + " " + State + " " + Address;

  String WeatherJSON = GetWeather(Location);

  DynamicJsonDocument jsonBufferWeather(1024);
  //  JsonObject& weatherRoot = jsonBufferWeather.parseObject(WeatherJSON);
    deserializeJson(jsonBufferWeather, WeatherJSON);


  double temperature = jsonBufferWeather["daily"]["data"][0]["temperatureLow"];
...
void PaperSignals::UmbrellaExecution(String JSONData)
{
  ...
  String iconWeather = jsonBufferWeather["daily"]["data"][0]["icon"];
...
void PaperSignals::RocketExecution(String JSONData)
{
  ...
  DynamicJsonDocument agencyBuffer(1024);
 //   JsonObject& agencyRoot = agencyBuffer.parseObject(JSONData);
   deserializeJson(agencyBuffer,JSONData);
  
  String curSpaceAgency = agencyBuffer["parameters"]["SpaceAgency"];
  String curCountry = agencyBuffer["parameters"]["RocketCountryCodes"];
  ...
  DynamicJsonDocument jsonBufferRockets(1024);
 //   JsonObject& rocketRoot = jsonBufferRockets.parseObject(rocketPayload);
    deserializeJson(jsonBufferRockets, rocketPayload);
  String output = jsonBufferRockets["launches"][0]["isonet"];
  ...
  if(curSpaceAgency != "" || curCountry != "")
  {
    ...
    do
    {
      String countryCode = jsonBufferRockets["launches"][0]["location"]["pads"][0]["agencies"][spaceAgencyNum]["countryCode"];
      String agencyNew = jsonBufferRockets["launches"][0]["location"]["pads"][0]["agencies"][spaceAgencyNum]["abbrev"];
      agency = agencyNew;
...
{
  DynamicJsonDocument countDownBuffer(1024);
  //  JsonObject& countDownRoot = countDownBuffer.parseObject(JSONData);
  deserializeJson(countDownBuffer, JSONData);
  
  String timeStamp = countDownBuffer["timestamp"];
  ...
  String destTime = countDownBuffer["parameters"]["date"];
...
void PaperSignals::StockExecution(String JSONData)
{
  ...
  DynamicJsonDocument stockBuffer(1024);
  //  JsonObject& stockRoot = stockBuffer.parseObject(JSONData);
     deserializeJson(stockBuffer, JSONData);
  
  String StockSymbol = stockBuffer["parameters"]["StockSymbol"];
  ...
  //  JsonObject& barChartStockRoot = stockBuffer.parseObject(barChartStockJSON);
    ...
    deserializeJson(stockBuffer, barChartStockJSON);
    
  double stockNumber = stockBuffer["results"][0]["netChange"];
...
{
  DynamicJsonDocument customIntentBuffer(1024);
 //   JsonObject& customIntentRoot = customIntentBuffer.parseObject(JSONData);
     deserializeJson(customIntentBuffer, JSONData);
  
  String customIntentData = customIntentBuffer["parameters"]["customParameter"];
...
String PaperSignals::getSignalByID(String signalID){
    ...
    Serial.println("getSignalByID called");
   ...
   //   JsonObject& root = jsonBuffer.parseObject(payload);
    ...
    {
      String signalInfo = jsonBuffer["result"];
      String intentName = jsonBuffer["result"]["metadata"]["intentName"];
      String intentTimeStamp = jsonBuffer["result"]["timestamp"];
...
String PaperSignals::getJson(String host, String url){
    ...
    Serial.println(host);
    client.setInsecure();//this line was key!!!
...
{
  Serial.println("RunPaperSignals called...");
  Serial.println(SignalID.c_str());
